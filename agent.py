...
from sandbox.features import FLAP, IDLE, FlappyObservation, player_y, screen_height, next_gap_center, player_velocity, next_pipe
...
class Agent:
    ...
    def act(self, observation: FlappyObservation) -> int:
        ...
        # means lower on the screen.pyt
        #below_middle = player_y(observation) > screen_height(observation) / 2
        below_middle = (player_y(observation)) > next_gap_center(observation)
